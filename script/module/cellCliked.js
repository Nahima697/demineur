

import { hasnoMines } from "./hasnoMines.js";
import { Game } from "./game.js";

// gestion des intéractions au Click.
function cellClicked(e, newData) {
  let cell = e.target;
  const row = parseInt(cell.dataset.row);
  const col = parseInt(cell.dataset.col);
  const cellValue = cell.dataset.value;
  const isMined = newData[row][col].isMined;

  const game = new Game(); // Créez une instance de la classe Game

  if (e.button === 2) { // Clic droit
    e.preventDefault(); // Empêche le menu contextuel de s'afficher
    if (!cell.classList.contains('flag')) {
      cell.classList.add('flag');
    } else {
      cell.classList.remove('flag');
    }
  } else { // Click gauche
    cell.innerHTML = cellValue;
    cell.revealed = true;

    if (isMined === true) {
      cell.innerHTML = "";
      cell.classList.add('boum');
      newData[row][col].endGame = true;
      game.endGame(false); // Appeler la fonction endGame de la classe Game
    } else if (cellValue === '0') {
      hasnoMines(newData, row, col);
      game.checkGameStatus(newData); // Vérifier l'état du jeu
    }
  }
}

export { cellClicked };
